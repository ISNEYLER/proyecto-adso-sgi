...
<?php echo $this->section('content'); ?>
    ...
    <div class="flex justify-between items-center p-4">
        <h4 class="text-xl font-semibold text-gray-800">Editar categoria</h4>
    ...
    <div class="bg-white rounded-xl shadow p-6 mt-4">
        <form id="formCreateCategory" action="<?= base_url('categories/update/'.$category->id) ?>" method="post" autocomplete="off" class="grid grid-cols-1 md:grid-cols-12 gap-4">
            <!-- Producto -->
            <div class="md:col-span-12">
                <label class="block text-sm font-medium text-gray-700 mb-1">Nombre de la Categoria</label>
                <input type="text" name="nombre" placeholder="Ropa" value="<?= $category->nombre ?>" class="w-full rounded-lg border px-3 py-2 focus:outline-none focus:ring-2">
            </div>
        </form>
    </div>
</div>
